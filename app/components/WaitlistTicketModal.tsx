  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Gagal menambahkan tiket waitlist');
      }
    } catch (err) {
      console.error('Error creating waitlist tickets:', err);
      setError(err instanceof Error ? err.message : 'Gagal menambahkan tiket waitlist');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white px-3 py-3 md:px-6 md:py-4 border-b border-gray-200 z-10">
          <div className="flex justify-between items-center">
            <h2 className="text-lg md:text-xl font-semibold">Tambah Tiket Waitlist</h2>
            <button 
              onClick={onClose}
        ...
        <div className="px-3 py-4 md:px-6 md:py-6">
          <p className="text-sm md:text-base text-gray-700 mb-4">
            Tambahkan tiket waitlist untuk event ini. Tiket waitlist dapat dibuat jika semua tiket utama sudah habis terjual.
          </p>
          ...
          {success && (
            <div className="bg-green-50 p-3 rounded-md border border-green-200 mb-4">
              <p className="text-xs md:text-sm text-green-800">
                Tiket waitlist berhasil ditambahkan!
              </p>
            </div>
          )}
            ...
            {waitlistTickets.map((ticket, index) => (
              <div key={index} className="border border-gray-200 rounded-md p-3 md:p-4 mb-4">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-base md:text-lg font-medium">Tiket Waitlist #{index + 1}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div className="mb-2 md:mb-3">
                    <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">
                      Nama Tiket Waitlist <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={ticket.name}
                      onChange={(e) => handleTicketChange(index, 'name', e.target.value)}
                      placeholder="Contoh: VIP Waitlist"
                  ...
                  <div className="mb-2 md:mb-3">
                    <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">
                      Referensi Tiket Asli <span className="text-red-500">*</span>
                    </label>
                    <select
                      ...
                      required
                    >
                      <option value="">Pilih Tiket Asli</option>
                  ...
                  <div className="mb-2 md:mb-3">
                    ...
                    <input
                      ...
                      placeholder="Deskripsi tiket waitlist"
                      className="w-full p-2 text-sm border border-gray-300 rounded-md"
                    />
                  </div>
                  ...
                  <div className="mb-2 md:mb-3">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Kosongkan untuk menggunakan harga tiket asli
                    </p>
                  </div>
                  ...
                  <div className="mb-2 md:mb-3">
                    ...
                    <input
                      ...
                      placeholder="Jumlah tiket waitlist"
                      className="w-full p-2 text-sm border border-gray-300 rounded-md"
                      min="1"
                      required
                    />
